...
public class TagitFactory {
	...
	private MeshComponent newArrow(float x, float y, float height, Color top,
		...
		TagitFactory.getInstance().addRotateAnimation(pyr, 120, new Vec(0, 0, 1));
	...
	public Obj newSolarSystem(Vec position) {
		...
		MeshComponent earth = TagitFactory.getInstance().newCircle(Color.green());
	...
	public MeshComponent newUndirectedPath(GeoObj from, GeoObj to, Color color) {
		return TagitFactory.getInstance().newUndirectedPath(
